    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          {summary.city} · {summary.area_m2?.toLocaleString()} m² · {summary.floors} floors · Live
          building overview
        </p>
      </div>

      <div>
        <h2 className="text-xs font-medium text-emerald-800/60 uppercase tracking-wide mb-2">
          Performance pillars
        </h2>
        <PerformancePillarsStrip
          metrics={{
            energy_label: `Solar + peak shave · ${k.savings_pct_week ?? 30}% week cut`,
            iaq_label: "IAQ OK · CO2 dilution on demand",
            water_label: "Cooling tower aware · greywater ready",
            carbon_label: `${k.co2_avoided_kg ?? 338} kg CO2 avoided today`,
            space_label: `${k.occupied_pct ?? 58}% zone use · daylight harvest`,
          }}
        />
        <p className="text-[10px] text-slate-400 mt-2">
          Aligned with ECBC commercial baselines, occupant comfort, and measurable kWh /
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <Kpi
          ...
          label="INR saved today"
          value={`₹${k.savings_today_inr?.toLocaleString()}`}
          accent
        />
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <ModuleCard
            ...
            status={m.occupancy_hvac?.status}
            primary={m.occupancy_hvac?.label}
            onClick={() => navigate("/occupancy")}
          />
          <ModuleCard
            title="ECBC Digital Twin"
            status={m.digital_twin?.status}
            primary={`EPI ${m.digital_twin?.epi}`}
            secondary={`Target ${m.digital_twin?.target_epi}`}
            detail={m.digital_twin?.label}
            onClick={() => navigate("/digital-twin")}
          />
          <ModuleCard
            title="Fault Detection"
            status={m.fault_detection?.status}
            primary={m.fault_detection?.label}
            onClick={() => navigate("/faults")}
          />
          <ModuleCard
            title="Solar & Grid"
            status={m.grid_solar?.status}
            primary={`${m.grid_solar?.self_consumption_pct}% self-use`}
            secondary={`Battery ${m.grid_solar?.battery_soc_pct}%`}
            onClick={() => navigate("/grid-solar")}
          />
          <ModuleCard
    ...
    >
      ...
      <p
        className={`text-lg font-semibold tracking-tight ${
          warn ? "text-amber-800" : accent ? "text-emerald-800" : "text-slate-900"
        }`}
      >
        {value}
      </p>
